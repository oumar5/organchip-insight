...
| Élément | Valeur |
|---|---|
| Code évalué | commit `d39e5019632eec96d3f48c99c47c16558087d4cc` |
...
exacte du protocole et un reçu d'accès unique. Le test est conçu pour être
ouvert une seule fois, après le gel du modèle, du seuil et du checkpoint.
...
```text
data/experiments/ooc-cnn/smoke-local-d39e501/best-checkpoint.pt
...
```text
data/experiments/ooc-cnn/smoke-local-d39e501/validation-report.json
SHA-256 9607e7a9a1cd87a183ad81357f9bf671bbef36e17e86af8f629540c7ec34e4fe
```
...
```text
data/experiments/ooc-cnn/smoke-local-d39e501/onnx/model.onnx
...
`/kaggle/working/organchip-insight` : code, configuration, contrat runtime,
...
Le préflight doit vérifier le contrat runtime et tous les hashes avant de
lancer le GPU.
...
protocole persiste d'abord le reçu unique, puis seulement le CLI ouvre le
manifeste et vérifie son SHA-256.
...
- ne déclencher l'accès test unique qu'après revue et gel formel de la
  validation, du seuil, du checkpoint et des hashes.

## Artefacts locaux

- rapport miroir : `reports/generated/ooc-cnn/smoke-local-d39e501.json` ;
- dossier du run : `data/experiments/ooc-cnn/smoke-local-d39e501/` ;
- rapport d'export :
  `data/experiments/ooc-cnn/smoke-local-d39e501/onnx/export-report.json`.
