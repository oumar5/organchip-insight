...
def check_notebook(path: Path) -> None:
    ...
    _require(
        ...
        (
            ...
            'FINAL_EVAL_REASON = ""',
            "def mounted_dataset_root(slug: str) -> Path:",
            'Path("/kaggle/input") / slug',
            'Path("/kaggle/input/datasets/oumarbenlol") / slug',
            'SOURCE_DATASET_ROOT = mounted_dataset_root("organchip-insight-source-campaign-v2")',
            'SOURCE_INPUT_DIR = SOURCE_DATASET_ROOT / "organchip-insight"',
            'SOURCE_MANIFEST_PATH = SOURCE_DATASET_ROOT / "source-bundle-manifest.json"',
            "EXPECTED_SOURCE_BUNDLE_SHA256",
            'TRAIN_VALIDATION_IMAGE_ROOT = mounted_dataset_root("organchip-train-validation-v2")',
            'WEIGHTS_INPUT_DIR = mounted_dataset_root("organchip-cnn-offline-resources-v1")',
